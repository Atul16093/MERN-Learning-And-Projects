...
//   console.log("Member " , sendToChild);
  useEffect(() => {
      fetchData();
  }, []);
    ...
    <div className="modal-overlay" onClick={onClose}>
      ...
      >
        ...
        <div className="members-section">
          ...
          {members.map((member) => (
            <label key={member.user._id} className="member-item">
              ...
              />
              <img src={`http://localhost:5400${member.user.profilePic}`} alt={member.name} className="avatar" />
              {member.user.username}{member.role == "admin" ? <span style={{fontSize : "10px" , marginTop : "10px"}}>(Admin)</span> : ""}
            </label>
          ))}
        </div>
        ...
        <div className="buttons">
          ...
          <button className="skip-btn" onClick={handleAddMember}>
            Create
          </button>
        </div>
      </div>
